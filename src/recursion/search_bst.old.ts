  ...
  function dfs(sliced_arr: number[], target: number): undefined | number {
    ...
    if (ARRAY_CONTAINS_SINGLE_ELEMENT) {
      return index = target === sliced_arr[0] ? nums.indexOf(target) : -1;
        ...
        // edge-cases
        if( midpoint === target ) return index = nums.indexOf(midpoint)
        if( greatest_of_first_arr === target ) return index = nums.indexOf(greatest_of_first_arr)

        // recursive case 
        if(greatest_of_first_arr > target) return dfs(first_arr, target)
